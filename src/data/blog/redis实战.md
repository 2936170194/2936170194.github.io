...
- **主动更新**（推荐）：适用于高一致性需求场景
  - 实现方式：先更新数据库，再删除缓存
  - 原理：避免无效写操作（直接更新缓存可能导致中间更新无意义），保证缓存与数据库最终一致
  - 注意：需确保数据库与缓存操作的原子性（单体系统可使用事务，分布式系统需使用TCC等方案）

#### 1.2 缓存问题及解决方案
- **缓存穿透**：请求不存在的数据导致绕过缓存直接查询数据库
  - 解决方案：缓存空对象（设置较短过期时间，如1分钟），避免重复查询不存在的数据
- **缓存雪崩**：大量缓存同时过期导致数据库压力骤增
  - 解决方案：设置随机过期时间，避免缓存集中失效
- **缓存击穿**：热点数据缓存过期时，大量请求同时查询数据库
  - 解决方案：
    - 互斥锁：保证同一时间只有一个线程重建缓存，其他线程等待
    - 逻辑过期：缓存数据设置逻辑过期时间，过期后异步重建，避免请求阻塞

#### 1.3 Redis工具类封装
- 通用缓存操作方法：支持序列化/反序列化任意Java对象
- 缓存穿透解决方案：查询时缓存空对象
- 缓存击穿解决方案：逻辑过期+异步重建缓存

### 2. 分布式锁实现

#### 2.1 基于Redis的分布式锁原理
- **核心命令**：`SETNX key value EX timeout`（原子操作，确保互斥性和过期时间）
- **释放锁**：需先检查线程标识，再删除锁（避免误删其他线程的锁）
- **原子性保证**：使用Lua脚本执行释放锁操作，确保检查和删除的原子性

#### 2.2 Redisson高级分布式锁
- **可重入锁**：基于Hash结构存储线程标识和重入次数，支持同一线程多次获取锁
- **重试机制**：获取锁失败时，通过信号量等待锁释放后重试
- **超时续约**：Watchdog机制自动延长锁时间（默认30秒过期，每10秒续约），避免业务未完成时锁过期
- **主从一致性**：使用MultiLock（联锁），需在多个Redis节点同时获取锁，避免主节点宕机导致锁丢失

### 3. 秒杀优化

#### 3.1 全局唯一ID生成
- **实现原理**：基于Redis自增功能，组合时间戳（31位，支持69年）和序列号（32位，支持每秒2^32个ID）
- **优势**：避免ID规律性，支持高并发，分布式环境唯一

#### 3.2 超卖问题解决
- **乐观锁**：更新库存时添加条件 `stock > 0`，确保库存不为负
- **一人一单**：使用分布式锁（基于用户ID），确保每个用户只能购买一次

#### 3.3 异步秒杀架构
- **资格认证**：使用Lua脚本原子操作检查库存和用户购买记录
- **异步处理**：秒杀成功后将订单信息放入阻塞队列，后台线程异步创建订单
- **优势**：减少数据库压力，提高系统并发能力

### 4. 核心技术要点

#### 4.1 Redis数据结构应用
- **String**：存储验证码、库存等简单数据
- **Hash**：存储用户信息等复杂对象（节省空间，支持部分字段修改）
- **Set**：存储用户购买记录（用于一人一单校验）

#### 4.2 Lua脚本
- **作用**：保证多个Redis命令的原子性执行
- **应用场景**：秒杀资格判断、分布式锁释放等需要原子操作的场景

#### 4.3 性能优化策略
- **缓存**：减少数据库访问，提高响应速度
- **异步处理**：将耗时操作（如订单创建）异步执行
- **分布式锁**：控制并发访问，避免资源竞争

### 5. 最佳实践
- **缓存策略**：根据业务一致性需求选择合适的缓存更新策略
- **分布式锁**：优先使用Redisson，简化开发并解决常见问题
- **秒杀架构**：结合Redis原子操作和异步处理，提高系统并发能力
- **代码封装**：将Redis操作封装为工具类，提高代码复用性和可维护性
...
- 利用Redis的高效、高并发的特性使用lua脚本进行原子操作，对用户进行秒杀资格认证(库存和一人一单)，将(优惠卷id、用户id、订单id)保存到redis，返回订单id，供用户付款。之后可以异步慢慢处理订单创建操作。
  ...
  ![优惠卷秒杀业务优化](../../../public/blog/redis实战/43.jpg)
  ...
  - `步骤一：`创建阻塞队列
  - 阻塞队列有一个特点：当一个线程尝试从阻塞队列里获取元素的时候，如果没有元素，那么该线程就会被阻塞，直到队列中有元素，才会被唤醒，并去获取元素。阻塞队列的创建需要指定一个大小
  ...
  - 创建线程任务，秒杀业务需要在类初始化之后，就立即执行，所以这里需要用到`@PostConstruct`注解
  ...
  public class VoucherOrderServiceImpl extends ServiceImpl<VoucherOrderMapper, VoucherOrder> implements IVoucherOrderService {
      ...
      private static final DefaultRedisScript<Long> SECKILL_SCRIPT;//Long是lua脚本的返回值类型
  
      static {//静态代码块，在类加载时执行一次，用于初始化UNLOCK_SCRIPT
          SECKILL_SCRIPT = new DefaultRedisScript<>();
          SECKILL_SCRIPT.setLocation(new ClassPathResource("seckill.lua"));//ClassPathResource()会去classpath（就是resources目录）下寻找unlock.lua文件
      ...
      private void handleVoucherOrder(VoucherOrder voucherOrder) {
          ...
          try {
              proxy.createVoucherOrder(voucherOrder);//这种是代理对象的方法调用，事务生效
          } finally {
              lock.unlock(); //释放锁
          }
      }
      ...
      public void createVoucherOrder(VoucherOrder voucherOrder) {
          //5.一人一单
          Long userId = voucherOrder.getUserId();//从用户登录拦截器中获取用户id
          //5.1. 查询订单
  ...
  ```
![Redis秒杀优化总结](../../../public/blog/redis实战/44.jpg)

## 7. Redis消息队列实现异步秒杀
### 7.1 认识消息队列
![Redis消息队列实现异步秒杀](../../../public/blog/redis实战/45.jpg)
![Redis消息队列实现异步秒杀](../../../public/blog/redis实战/58.jpg)
### 7.2 基于List结构模拟消息队列
![Redis消息队列实现异步秒杀](../../../public/blog/redis实战/46.jpg)
![Redis消息队列实现异步秒杀](../../../public/blog/redis实战/47.jpg)
### 7.3 PubSub实现消息队列
![Redis消息队列实现异步秒杀](../../../public/blog/redis实战/48.jpg)
![Redis消息队列实现异步秒杀](../../../public/blog/redis实战/49.jpg)
### 7.4 基于Stream实现消息队列
发送消息
![Redis消息队列实现异步秒杀](../../../public/blog/redis实战/50.jpg)
接收消息
![Redis消息队列实现异步秒杀](../../../public/blog/redis实战/51.jpg)
![Redis消息队列实现异步秒杀](../../../public/blog/redis实战/52.jpg)
![Redis消息队列实现异步秒杀](../../../public/blog/redis实战/53.jpg)
消费者组
![Redis消息队列实现异步秒杀](../../../public/blog/redis实战/54.jpg)
![Redis消息队列实现异步秒杀](../../../public/blog/redis实战/55.jpg)
![Redis消息队列实现异步秒杀](../../../public/blog/redis实战/56.jpg)
消费者监听消息的基本思路
```java
while(true){
...
```
![Redis消息队列实现异步秒杀](../../../public/blog/redis实战/57.jpg)
